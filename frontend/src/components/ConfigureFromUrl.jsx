    ...
    try {
      ...
      if (parsedData.api && typeof parsedData.api === "object") {
        ...
        if (
          ...
          localStorage.setItem(
            ...
            JSON.stringify(apiConfigToSave),
          );
          console.log("Applied API configuration from URL:", apiConfigToSave);
          configApplied = true;
        } else {
          console.warn(
            "Invalid API configuration structure in URL data:",
            parsedData.api,
          );
      ...
      if (parsedData.locale && typeof parsedData.locale === "string") {
        ...
        if (validLocales.includes(parsedData.locale)) {
          ...
          console.log(
            "Applied Locale configuration from URL:",
            parsedData.locale,
          );
          configApplied = true;
        } else {
      ...
      if (configApplied) {
        ...
        setStatusMessage(
          intl.formatMessage({
            id: "configure.error.noValidData",
            defaultMessage:
              "Error: No valid configuration data found in the provided values.",
          }),
        );
        setIsError(true);
      }
      ...
      if (error instanceof SyntaxError) {
        ...
        defaultMessage =
          "Error: Could not parse configuration data (Invalid JSON).";
