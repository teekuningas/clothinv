...
const ExportConfigurationLink = () => {
  ...
  useEffect(() => {
    ...
    if (providerType) {
      ...
      if (provider) {
        ...
        setError(
          intl.formatMessage(
            {
              id: "exportConfig.error.invalidProvider",
              defaultMessage:
                "Error: Invalid provider type specified: {providerType}",
            },
            { providerType },
          ),
        );
    ...
    if (locale) {
      ...
      if (validLocales.includes(locale)) {
        ...
        setError(
          intl.formatMessage(
            {
              id: "exportConfig.error.invalidLocale",
              defaultMessage: "Error: Invalid locale specified: {locale}",
            },
            { locale },
          ),
        );
    ...
    if (!configToEncode.api && !configToEncode.locale) {
      setError(
        intl.formatMessage({
          id: "exportConfig.error.noParams",
          defaultMessage:
            "Error: No configuration parameters (providerType, setting_*, locale) provided in the URL.",
        }),
      );
      return;
    }
  ...
  const handleCopyEncoded = () => {
    navigator.clipboard
      ...
      .then(() =>
        alert(
          intl.formatMessage({
            id: "exportConfig.alert.encodedCopied",
            defaultMessage: "Encoded string copied to clipboard!",
          }),
        ),
      )
      .catch((err) =>
        alert(
          intl.formatMessage(
            {
              id: "exportConfig.alert.copyFailed",
              defaultMessage: "Failed to copy string: {error}",
            },
            { error: err },
          ),
        ),
    ...
    <div className="settings-view">
      ...
      {!error && generatedUrl && (
        <div className="settings-fieldset">
          ...
          <h3>
            {intl.formatMessage({
              id: "exportConfig.section.details",
              defaultMessage: "Configuration Details Included:",
            })}
          </h3>
          {configDetails.api && (
            <div>
              <strong>
                {intl.formatMessage({
                  id: "exportConfig.details.apiProvider",
                  defaultMessage: "API Provider:",
                })}
              </strong>{" "}
              {configDetails.api.providerType}
              <ul>
                {Object.entries(configDetails.api.settings).map(
                  ([key, value]) => (
                    <li key={key}>
                      <code>{key}</code>:{" "}
                      <code>
          ...
          {configDetails.locale && (
            <p>
              <strong>
                {intl.formatMessage({
                  id: "exportConfig.details.language",
          ...
          <h3>
            {intl.formatMessage({
              id: "exportConfig.section.encodedString",
              defaultMessage: "Base64 Encoded String:",
            })}
          </h3>
          <p
            style={{
          ...
          <button onClick={handleCopyEncoded} className="button-primary">
            {intl.formatMessage({
              id: "exportConfig.button.copyEncoded",
              defaultMessage: "Copy Encoded String",
            })}
          </button>
          ...
          <p className="status-warning" style={{ marginTop: "10px" }}>
            {intl.formatMessage({
              ...
              defaultMessage:
                "Warning: This URL contains configuration details, potentially including API tokens. Share it only with trusted individuals.",
            })}
          </p>
        </div>
      )}
      ...
      <div className="settings-fieldset" style={{ marginTop: "30px" }}>
        <h4>
          {intl.formatMessage({
            id: "exportConfig.section.usage",
            defaultMessage: "How to Use This Page",
          })}
        </h4>
        <p>
          {intl.formatMessage({
            id: "exportConfig.usage.description",
            defaultMessage:
              "Construct a URL pointing to this page with query parameters representing the desired configuration. Example:",
          })}
        </p>
        <code
          style={{
        ...
        >
          ...
          /export-configuration?providerType=datasette&amp;setting_datasetteBaseUrl=https://your-datasette.example.com/inventory&amp;setting_datasetteApiToken=YOUR_SECRET_TOKEN&amp;locale=en
        </code>
        <p style={{ marginTop: "10px" }}>
          {intl.formatMessage({
            id: "exportConfig.usage.parameters",
        ...
        <ul>
          <li>
            ...
            {intl.formatMessage({
              id: "exportConfig.param.providerType",
              defaultMessage:
                "The ID of the API provider (e.g., datasette, localStorage).",
            })}
          </li>
          <li>
            <code>setting_KEY</code>:{" "}
            {intl.formatMessage({
              id: "exportConfig.param.setting",
              defaultMessage:
                "Replace KEY with the specific setting name for the provider (e.g., setting_datasetteBaseUrl, setting_datasetteApiToken).",
            })}
          </li>
          <li>
            <code>locale</code>:{" "}
            {intl.formatMessage({
              id: "exportConfig.param.locale",
              defaultMessage: "The desired language code (e.g., en, fi).",
            })}
          </li>
        </ul>
      </div>
