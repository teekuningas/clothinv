...
const WebcamCapture = ({ show, onCapture, onClose }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            console.log("WebcamCapture unmounting or show changed to false, stopping camera."); // Debug log
            stopCamera();
        };
    ...
    const handleCaptureClick = useCallback(() => {
        if (!videoRef.current || !canvasRef.current || !stream) {
            console.error("Capture cannot proceed: video or canvas ref not ready, or no stream.");
            return;
        }
    ...
    return (
        <div className="webcam-capture-backdrop" onClick={onClose}>
            <div className="webcam-capture-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="webcam-video-container">
                    <video
                        ...
                        style={{ visibility: !isInitializing && !error && stream ? 'visible' : 'hidden' }}
                    />
                    ...
                    {stream && !isInitializing && !error && ( // Condition remains the same
                         <button
