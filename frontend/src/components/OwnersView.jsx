...
const OwnersView = () => {
    ...
    const fetchOwners = useCallback(async () => {
        ...
        if (!api.config.isConfigured || typeof api.listOwners !== 'function') {
            ...
            setError(api.config.isConfigured
                ? intl.formatMessage({ id: 'owners.list.notSupported', defaultMessage: 'Listing owners is not supported by the current API Provider.' })
        ...
        try {
            ...
            setError(intl.formatMessage({ id: 'owners.error.fetch', defaultMessage: 'Failed to fetch owners: {error}' }, { error: err.message }));
    ...
    const handleAddOwner = async (e) => { // Make async
        e.preventDefault(); // Prevent default form submission

        if (!newOwnerName.trim()) {
            setError(intl.formatMessage({ id: 'owners.error.nameEmpty', defaultMessage: 'Owner name cannot be empty.' }));
            return;
        }
        // Only add if the provider is configured and addOwner exists
        if (!api.config.isConfigured || typeof api.addOwner !== 'function') {
             setError(api.config.isConfigured
                ? intl.formatMessage({ id: 'owners.addForm.notSupported', defaultMessage: 'Adding owners is not supported by the current API Provider.' })
                : intl.formatMessage({ id: 'common.status.apiNotConfigured', defaultMessage: 'API Provider is not configured. Please configure it in Settings.' })
             );
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                setSuccess(intl.formatMessage({ id: 'owners.success.add', defaultMessage: 'Owner "{name}" added successfully!' }, { name: newOwnerName.trim() }));
                setNewOwnerName('');
                ...
                setError(intl.formatMessage({ id: 'owners.error.add', defaultMessage: 'Failed to add owner: {error}' }, { error: result.message || intl.formatMessage({ id: 'common.error.unknown', defaultMessage: 'Unknown reason' }) }));
            }
        } catch (err) {
            ...
            setError(intl.formatMessage({ id: 'owners.error.add', defaultMessage: 'Failed to add owner: {error}' }, { error: err.message }));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdateOwner = async (e) => { // Make async
        e.preventDefault();
        if (!editingOwnerId || !editName.trim() || typeof api.updateOwner !== 'function') {
            setUpdateError(intl.formatMessage({ id: 'owners.error.updateInvalid', defaultMessage: 'Cannot update. Invalid data or update function unavailable.' }));
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                setSuccess(intl.formatMessage({ id: 'owners.success.update', defaultMessage: 'Owner "{name}" updated successfully!' }, { name: editName.trim() }));
                handleCancelEdit(); // Close modal
                fetchOwners(); // Refresh list
            } else {
                // Should ideally not happen if updateOwner throws errors
                setUpdateError(intl.formatMessage({ id: 'owners.error.update', defaultMessage: 'Failed to update owner: {error}' }, { error: result.message || intl.formatMessage({ id: 'common.error.unknown', defaultMessage: 'Unknown reason' }) }));
            }
        } catch (err) {
            console.error("Failed to update owner:", err); // Keep console error in English
            // Use intl for consistency
            setUpdateError(intl.formatMessage({ id: 'owners.error.update', defaultMessage: 'Failed to update owner: {error}' }, { error: err.message }));
        } finally {
    ...
    const handleConfirmDelete = async () => { // Make async
        if (!deleteCandidateId || typeof api.deleteOwner !== 'function' || typeof api.listItems !== 'function') {
            setDeleteError(intl.formatMessage({ id: 'owners.error.deleteInvalid', defaultMessage: 'Cannot delete. Invalid data or required API functions unavailable.' }));
            return;
        }
        ...
        try {
            ...
            const isOwnerInUse = items.some(item => item.owner_id === deleteCandidateId);

            if (isOwnerInUse) {
                throw new Error(intl.formatMessage({ id: 'owners.error.deleteInUse', defaultMessage: 'Cannot delete owner because it is currently assigned to one or more items.' }));
            }

            // 2. Proceed with deletion if not in use
            const result = await api.deleteOwner(deleteCandidateId);
            if (result.success) {
                setSuccess(intl.formatMessage({ id: 'owners.success.delete', defaultMessage: 'Owner deleted successfully!' }));
                ...
                setDeleteError(intl.formatMessage({ id: 'owners.error.delete', defaultMessage: 'Failed to delete owner: {error}' }, { error: result.message || intl.formatMessage({ id: 'common.error.unknown', defaultMessage: 'Unknown reason' }) }));
            }
            ...
            const errorMessage = isAlreadyTranslated
                ...
                : intl.formatMessage({ id: 'owners.error.delete', defaultMessage: 'Failed to delete owner: {error}' }, { error: err.message });
    ...
    return (
        <div className="owners-view"> {/* Use owners-view class */}
            <h2>{intl.formatMessage({ id: 'owners.title', defaultMessage: 'Owners Management' })}</h2>

            {/* Status Messages */}
            {loading && <p className="status-loading">{intl.formatMessage({ id: 'owners.loading', defaultMessage: 'Loading owners...' })}</p>}
            ...
            {!api.config.isConfigured ? (
                 ...
                 <p className="status-warning">{intl.formatMessage({ id: 'owners.addForm.notSupported', defaultMessage: 'Adding owners is not supported by the current API Provider.' })}</p>
            ) : (
                <form onSubmit={handleAddOwner} className="add-owner-form"> {/* Use add-owner-form class */}
                    <h3>{intl.formatMessage({ id: 'owners.addForm.title', defaultMessage: 'Add New Owner' })}</h3>
                    <div className="form-group">
                        <label htmlFor="owner-name">{intl.formatMessage({ id: 'owners.addForm.nameLabel', defaultMessage: 'Name:' })}</label>
                        <input
                            type="text"
                            id="owner-name"
                            value={newOwnerName}
                    ...
                    <div className="form-group">
                        <label htmlFor="owner-description">{intl.formatMessage({ id: 'owners.addForm.descriptionLabel', defaultMessage: 'Description:' })}</label>
                        <input
                            type="text"
            ...
            {/* Owners List */}
            <h3>{intl.formatMessage({ id: 'owners.list.title', defaultMessage: 'Existing Owners' })}</h3>
            {typeof api.listOwners !== 'function' && api.config.isConfigured && (
                 <p className="status-warning">{intl.formatMessage({ id: 'owners.list.notSupported', defaultMessage: 'Listing owners is not supported by the current API Provider.' })}</p>
            )}
            {typeof api.listOwners === 'function' && !loading && owners.length === 0 && !error && api.config.isConfigured && (
                <p>{intl.formatMessage({ id: 'owners.list.empty', defaultMessage: 'No owners found. Add one above!' })}</p>
            )}
            {typeof api.listOwners === 'function' && owners.length > 0 && (
                <div className="owners-list"> {/* Use div for card container */}
                    {owners.map((owner) => (
            ...
            {/* Edit Owner Modal */}
            {editingOwnerId && (
                <Modal show={!!editingOwnerId} onClose={handleCancelEdit} title={intl.formatMessage({ id: 'owners.editModal.title', defaultMessage: 'Edit Owner' })}>
                    <form onSubmit={handleUpdateOwner} className="edit-owner-form">
                        {updateError && <p className="status-error">Error: {updateError}</p>}
                        <div className="form-group">
                            <label htmlFor="edit-owner-name">{intl.formatMessage({ id: 'owners.addForm.nameLabel', defaultMessage: 'Name:' })}</label>
                            <input
                                type="text"
                                id="edit-owner-name"
                                value={editName}
                        ...
                        <div className="form-group">
                            <label htmlFor="edit-owner-description">{intl.formatMessage({ id: 'owners.addForm.descriptionLabel', defaultMessage: 'Description:' })}</label>
                            <input
                                type="text"
            ...
            {showDeleteConfirm && (
                <Modal show={showDeleteConfirm} onClose={handleCancelDelete} title={intl.formatMessage({ id: 'owners.deleteModal.title', defaultMessage: 'Confirm Deletion' })}>
                    <div className="delete-confirm-content">
                        {deleteError && <p className="status-error">Error: {deleteError}</p>}
                        <p>
                            {intl.formatMessage({ id: 'owners.deleteModal.confirmMessage', defaultMessage: 'Are you sure you want to delete the owner "{name}"? This action cannot be undone.' }, { name: owners.find(o => o.owner_id === deleteCandidateId)?.name || '' })}
                        </p>
